...
self.addEventListener('fetch',event=>{
  ...
  if(req.mode==='navigate'){
    event.respondWith(fetch(req).then(res=>{const clone=res.clone();caches.open(CACHE).then(c=>c.put(req,clone));return res}).catch(()=>caches.match(req).then(r=>r||caches.match('/'))));return;
  }
