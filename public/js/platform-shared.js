(()=>{
  ...
  const initialTheme=localStorage.getItem('theme')||(matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light');document.documentElement.dataset.theme=initialTheme;const initialCachedLogo=localStorage.getItem(initialTheme==='dark'?'yuran-header-logo-dark':'yuran-header-logo-light')||localStorage.getItem('yuran-header-logo')||'';const initialLogo=$('#sharedBrandLogo'),initialMark=$('#sharedBrandMark');if(initialMark)initialMark.hidden=true;if(initialLogo&&initialCachedLogo){initialLogo.src=initialCachedLogo;initialLogo.hidden=false;}
  async function applyBrand(){
    try{
      ...
      const applyLogo=()=>{const theme=document.documentElement.dataset.theme||'light',cached=localStorage.getItem(theme==='dark'?'yuran-header-logo-dark':'yuran-header-logo-light')||localStorage.getItem('yuran-header-logo')||'',headerLogo=theme==='dark'?(s.headerLogoDark||s.headerLogo||s.logo||cached||s.headerLogoLight):(s.headerLogoLight||s.headerLogo||s.logo||cached||s.headerLogoDark);if(logo&&mark){if(headerLogo){logo.src=headerLogo;logo.hidden=false;mark.hidden=true;try{localStorage.setItem(theme==='dark'?'yuran-header-logo-dark':'yuran-header-logo-light',headerLogo)}catch{}}else{logo.hidden=true;mark.hidden=true}}};
      applyLogo();window.__yuranApplySharedLogo=applyLogo;
  ...
  const saved=document.documentElement.dataset.theme||localStorage.getItem('theme')||(matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light');document.documentElement.dataset.theme=saved;
  const theme=$('#themeToggle'); if(theme)theme.onclick=()=>{const n=document.documentElement.dataset.theme==='dark'?'light':'dark';document.documentElement.dataset.theme=n;localStorage.setItem('theme',n);window.__yuranApplySharedLogo?.()};
